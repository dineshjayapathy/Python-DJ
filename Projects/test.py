...
def files():
    ...
    n = 0
    while True:
        with open('C:\DJ\LDH\misc\PRSEXT_Extract_Queries.txt') as i:
            for row in i:
                if re.findall(r'^from\s\w+', row):
                    a = ''.join(re.findall(r'^from\s\w+', row)).replace('from ', '')
                    #print a
                    yield open('C:\DJ\LDH\misc\op\%s.sql' % a, 'w')
pat = '-----------------------------------------------------------------------'
fs = files()

outfile = next(fs)
with open('C:\DJ\LDH\misc\PRSEXT_Extract_Queries.txt') as infile:
    for line in infile:
        if pat not in line:
