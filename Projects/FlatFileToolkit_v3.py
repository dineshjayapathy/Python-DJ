...
def mainfunction():
    try:
        ...
        if userinput == 'a':
            ...
            mainfunction()
        if userinput == 'b':
            #e=input('Enter the file path')
            #os.chdir(r'\\qdwsftp01\0921-PR-VIRDEE_SFTP_PRD')

            os.system('python csv_parse_dev.py')#python csv_parse_dev.py arg1 arg2 arg3

            #filepath = raw_input("Enter the SFTP folder \n")
            mainfunction()
            # \\qdwsftp01\0907-CMPS_SFTP_DEV
            #enc(filepath)
        if userinput == 'c':
